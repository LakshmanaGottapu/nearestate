...
const AddPropertyTabContent = (data) => {
  ...
  const listedIn = [
    ...
    { value: "6", label: "Villa" },
    { value: "18", label: "Commercial" },
    { value: "17", label: "Commercial space" },
    { value: "15", label: "Commercial Farm House" },
  ];
  ...
  useEffect(() => {
    if(data){
      // document.getElementById("propertyTitle").value=data.editData?.title;
      // tinyMCE.activeEditor.setContent(data.editData?.description);
      // setCategory(data.editData?.property_status);
    }
          ...
          if(_login.includes("Login")){
            ...
            .then(response => { console.log("response",response);
                if(response.status==200 && response?.data=="success"){
                  ...
                  alert("Error while inserting property.");
                }
            }).catch(error => {
              console.error(error);
            });
